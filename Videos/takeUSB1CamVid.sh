...
ffmpeg -i /dev/video1 -vf "drawtext=fontfile=/Windows/Fonts/Arial.ttf: text='%{localtime}': x=(w-tw)/2: y=h-(2*lh): fontcolor=white: box=1: boxcolor=0x00000000@1: fontsize=20" -b:v 192k -c:v libx264 -pix_fmt yuvj420p -preset ultrafast -r 15 -t $vidLength -y $vidPath
